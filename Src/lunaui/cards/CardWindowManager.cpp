...
void CardWindowManager::prepareAddWindow(Window* win)
{
	...
	if ((card->type() != Window::Type_Emulated_Card) && (card->hostWindowData() != 0) &&
...
{
	if (win && win->fullScreen() && (win->type() != Window::Type_Emulated_Card || win->type() != Window::Type_ModalChildWindowCard)) {
