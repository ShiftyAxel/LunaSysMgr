...
void WindowServer::tryTransmitEventForCes( QEvent* event )
{
	// [ CES Demo ]

	// Send events as packaged XML to an event repeater
	// for presntation.

	static struct sockaddr_in internetAddr;
	static int eventSocket = -1;

	if( -1 == eventSocket ) {
		memset( &internetAddr, 0, sizeof(internetAddr) );
		internetAddr.sin_addr.s_addr = inet_addr(Settings::LunaSettings()->cesGestureRepeaterIp.c_str());
		internetAddr.sin_family=AF_INET;
		internetAddr.sin_port=htons(Settings::LunaSettings()->cesGestureRepeaterPort);
		eventSocket = ::socket( PF_INET, SOCK_DGRAM, 0 );
	}
	if( eventSocket != -1 &&
		...
		case Qt::Key_CoreNavi_Back:
			if( event->type() == QEvent::KeyRelease ) {
				std::string data = "<relay><event gesture=\"back\" /></relay>";
				::sendto( eventSocket, data.c_str(), data.size()+1, 0,
					(struct sockaddr *)&internetAddr, sizeof(internetAddr) );
			}
			break;
		case Qt::Key_CoreNavi_Menu:
			if( event->type() == QEvent::KeyRelease ) {
				std::string data = "<relay><event gesture=\"menu\" /></relay>";
				::sendto( eventSocket, data.c_str(), data.size()+1, 0,
					(struct sockaddr *)&internetAddr, sizeof(internetAddr) );
			}
			break;
		case Qt::Key_CoreNavi_Next:
			if( event->type() == QEvent::KeyRelease ) {
				std::string data = "<relay><event gesture=\"next\" /></relay>";
				::sendto( eventSocket, data.c_str(), data.size()+1, 0,
					(struct sockaddr *)&internetAddr, sizeof(internetAddr) );
			}
			break;
		case Qt::Key_CoreNavi_Previous:
			if( event->type() == QEvent::KeyRelease ) {
				std::string data = "<relay><event gesture=\"previous\" /></relay>";
				::sendto( eventSocket, data.c_str(), data.size()+1, 0,
					(struct sockaddr *)&internetAddr, sizeof(internetAddr) );
			}
			break;
		case Qt::Key_CoreNavi_Launcher:
			if( event->type() == QEvent::KeyRelease ) {
				std::string data = "<relay><event x=\"0\" gesture=\"launcher\" /></relay>";
				::sendto( eventSocket, data.c_str(), data.size()+1, 0,
					(struct sockaddr *)&internetAddr, sizeof(internetAddr) );
			}
			break;
		case Qt::Key_CoreNavi_Home:
			{
				std::string data;
				if( event->type() == QEvent::KeyRelease )
					data= "<relay><event gesture=\"home-up\" /></relay>" ;
				else
					data= "<relay><event gesture=\"home-down\" /></relay>" ;
				::sendto( eventSocket, data.c_str(), data.size()+1, 0,
					(struct sockaddr *)&internetAddr, sizeof(internetAddr) );
			}
			break;
		case Qt::Key_CoreNavi_Meta:
			{
				std::string data;
				if( event->type() == QEvent::KeyRelease )
					data= std::string("<relay><event x=\"0\" gesture=\"meta-up\" /></relay>");
				else
					data= std::string("<relay><event x=\"0\" gesture=\"meta-down\" /></relay>");
				::sendto( eventSocket, data.c_str(), data.size()+1, 0,
					(struct sockaddr *)&internetAddr, sizeof(internetAddr) );
			}
			break;
...
bool WindowServer::handleEvent(QEvent* event)
{
	...
	//if( Settings::LunaSettings()->enableGestureRepeater )
	//	tryTransmitEventForCes(event);
...
void WindowServer::sceneTransitionPrepare(Window* win)
{
	if (!win || !windowIsRegistered(win))
		return;

	win->sceneTransitionPrepare();
}

void WindowServer::sceneTransitionRun(Window* win, const std::string& transitionName)
{
	if (!win || !windowIsRegistered(win))
		return;

	win->sceneTransitionRun(transitionName);
}
...
void WindowServer::takeAndSaveScreenShot()
{
	...
	std::string kScreenCapturesPathBase = "/tmp/captures/";
	mkdir(kScreenCapturesPathBase.c_str(), 777);
#else
	std::string kScreenCapturesPathBase = "/media/internal/screencaptures/";
	mkdir(kScreenCapturesPathBase.c_str(), 666);
#endif
	...
	while (1) {
		...
		fullPath = kScreenCapturesPathBase + appName + std::string(buffer) + ".png";
